 ...
 * @typedef {{ userId: string, role: 'super_admin'|'owner'|'staff', restaurantId: string|null, restaurantSlug: string|null, name: string, isImpersonating?: boolean }} SessionPayload
 */
...
export async function requireRestaurantAccess(targetRestaurantId) {
  ...
  const sessRestId = session.restaurantId?.toString();
  const targetRestId = targetRestaurantId?.toString();

  if (sessRestId && targetRestId && sessRestId === targetRestId) {
    return session;
  }

  // Fallback DB check: verify if logged in user owns targetRestaurantId
  if (session.userId && targetRestId) {
    await dbConnect();
    const Business = (await import('@/models/Business')).default;
    const biz = await Business.findOne({ _id: targetRestId, ownerId: session.userId }).select('_id');
    if (biz) {
      session.restaurantId = targetRestId;
      return session;
    }
  }
